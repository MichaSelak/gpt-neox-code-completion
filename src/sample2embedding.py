...
def input2tokens(lines: iter, tokenizer):
    result = []
    for line_id, cat, line in lines:
        line = line.strip()
        if line:
            result.append((line_id, cat, [tokenizer.tokenize(line)]))
    return result


def samples2input(samples: iter):
    for sample_id, _, sample_q, sample_a1, sample_a2, sample_a3 in samples:
        if not sample_q:
            continue
        yield sample_id, "q", sample_q
        yield sample_id, "a1", sample_a1
        if sample_a2:
            yield sample_id, "a2", sample_a2
        if sample_a3:
            yield sample_id, "a3", sample_a3
...
def main():
    ...
    token_input = input2tokens(inputs, neox_args.tokenizer)

    with open("embeddings.jsonl", "wt") as file:
        for sample_id, cat, context_tokens in token_input:
            sample_dir = {
                "id": sample_id,
                "category": cat
            }
            ...
            with torch.no_grad():
                ...
                if generated_token_logits.device == torch.device(0):
                    sample_dir["embedding"] = logits
                    file.write(json.dumps(sample_dir)+ "\n")
